...
class StaticChecker(BaseVisitor,Utils):
    ...
    def visitId(self, ast, c):
        ...
        for envi in c[0]:
            ...
            if sym is not None:
                IsDecl = True
                if sym.value is None and type(sym.mtype) is not MType:
    ...
    def visitCallExpr(self, ast, c):
        ...
        for envi in c[0]:
            sym = self.lookup(ast.method.name, envi, lambda x: x.name)
            if sym is not None:
                break
        if sym is None:
